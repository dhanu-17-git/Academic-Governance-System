...
CATEGORIES = ['Academic', 'Cleaning', 'Parking',
...
def admin_dashboard():
    ...
    return render_template('admin_dashboard.html', lab_summary=lab_summary, **dashboard_context)
...
def update_status(complaint_id):
    ...
    admin_response = sanitize_text(request.form.get('admin_response', ''), max_length=2000)
    ...
    success, err = complaint_service.update_complaint_status(clean_id, new_status, admin_response)
    if not success:
...
def manage_attendance():
    ...
    if request.method == 'POST':
        ...
        complaint_service.log_admin_action(session['user_email'], f'Updated attendance for {updated} records')
...
def manage_marks():
    ...
    if request.method == 'POST':
        ...
        complaint_service.log_admin_action(session['user_email'], f'Updated marks for {updated} records')
...
def delete_material(note_id):
    ...
    if academic_service.remove_note_with_file(note_id):
        complaint_service.log_admin_action(session['user_email'], f'Deleted note id={note_id}')
...
def delete_timetable_slot(slot_id):
    ...
    complaint_service.log_admin_action(session['user_email'], f'Deleted timetable slot id={slot_id}')
...
def update_lab_status(system_id):
    ...
    if success:
        complaint_service.log_admin_action(session['user_email'], f'Updated lab system id={system_id} → {new_status}')
...
def admin_notifications():
    ...
    if request.method == 'POST':
        ...
        if result['success']:
            complaint_service.log_admin_action(
                session['user_email'],
                f'Pushed notification: "{result["title"]}"'
            )
        flash(result['message'], result['flash_category'])
...
def bulk_create_students():
    ...
    if request.method == 'POST':
        result = academic_service.bulk_create_users_from_csv(request.files.get('csv_file'))
        if result['success']:
            complaint_service.log_admin_action(
                session['user_email'],
