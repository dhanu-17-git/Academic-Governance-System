...
from academic_governance.services.security import (
    ...
    validate_mime_type, rate_limiter,
)
...
_POSITIVE = {'good', 'great', 'excellent', 'amazing', 'wonderful',
             'best', 'love', 'like', 'happy', 'satisfied'}
_NEGATIVE = {'bad', 'poor', 'terrible', 'awful', 'worst',
             'hate', 'dislike', 'unhappy', 'disappointed', 'problem'}
...
def attendance_details(subject_id):
    ...
    context = academic_service.get_attendance_detail_context(session['user_email'], subject_id)
    if context is None:
...
def marks_details(subject_id):
    ...
    context = academic_service.get_marks_detail_context(session['user_email'], subject_id)
    if context is None:
...
def course_plan_details(subject_id):
    ...
    record = academic_service.get_student_attendance_record(session['user_email'], subject_id)
    if not record:
...
def raise_complaint():
    ...
    if request.method == 'POST':
        ...
        if not rate_limiter.is_allowed('complaint', email,
                                       config.RATE_COMPLAINT_MAX,
                                       config.RATE_COMPLAINT_WIN):
            flash('You have submitted too many complaints recently. Please try again later.', 'danger')
            return render_template('raise_complaint.html')

        # Sanitize & validate
        category    = sanitize_text(request.form.get('category', ''), max_length=50)
        description = sanitize_text(request.form.get('description', ''), max_length=2000)
        ...
        if 'file' in request.files:
            ...
            if file and file.filename:
                ...
                complaint_id = complaint_service.create_complaint(
                    ...
                    student_email=email, url=url_ref,
                )
        else:
            complaint_id = complaint_service.create_complaint(
                category=category, description=description,
        ...
        return redirect(url_for('student.complaint_confirmation',
...
def academic_feedback():
    ...
    if request.method == 'POST':
        ...
        if not rate_limiter.is_allowed('feedback', email,
                                       config.RATE_FEEDBACK_MAX,
                                       config.RATE_FEEDBACK_WIN):
            flash('You have submitted too much feedback recently. Please try again later.', 'danger')
