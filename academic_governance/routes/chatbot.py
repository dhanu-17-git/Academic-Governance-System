...
from academic_governance.services.chatbot_service import ask_gemini, build_student_context
...
def ask():
    ...
    if not student_email:
        return jsonify({"reply": "🔒 Please log in to use the Academic Advisor.", "status": "unauthorized"}), 401
    
    data = request.get_json(silent=True)
    if not data or not data.get("message", "").strip():
        return jsonify({"reply": "Please type a message first!", "status": "empty"}), 400
    ...
    try:
        ...
        return jsonify({"reply": f"⚠️ Could not load academic data: {str(e)}", "status": "db_error"}), 500

    try:
        ai_reply = ask_gemini(context, user_message)
    except Exception as e:
        return jsonify({"reply": "🤖 AI service unavailable.", "status": "ai_error"}), 500
