...
def main() -> None:
    with postgres_test_app():
        ...
        assert timetable, "Expected seeded timetable."
        ...
        assert labs, "Expected labs."
        layout = lab_service.get_lab_layout(lab.id)
        assert layout is not None and layout["rows"]
    ...
    print("test_orm_academic_lab_services.py: PASS")
