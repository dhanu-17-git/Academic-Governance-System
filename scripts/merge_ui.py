...
html = re.sub(
    r'<div class="px-4 mb-4">\s*<p class="text-\[10px\] font-bold text-slate-400 uppercase tracking-widest mb-2 px-2">Main Menu</p>\s*<nav class="space-y-1">.*?</nav>\s*</div>',
    f'<div class="px-4 mb-4"><p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 px-2">Main Menu</p>{sidebar_html}</div>',
    html,
    flags=re.DOTALL,
)

html = re.sub(
    r'<div class="px-4 mb-4">\s*<p class="text-\[10px\] font-bold text-slate-400 uppercase tracking-widest mb-2 px-2">Support</p>\s*<nav class="space-y-1">.*?</nav>\s*</div>',
    "",
    html,
    flags=re.DOTALL,
)
...
html = re.sub(
    r'<button class="text-slate-400 hover:text-slate-600">\s*<span class="material-symbols-outlined text-sm">logout</span>\s*</button>',
    '<a href="{{ url_for(\'auth.logout\') }}" class="text-rose-400 hover:text-rose-600 ml-auto"><span class="material-symbols-outlined text-sm">logout</span></a>',
    html,
)

# Top Navbar fixes
html = html.replace(
    '<div class="absolute top-2 right-2 h-2 w-2 bg-indigo-600 rounded-full"></div>',
...
html = html.replace(
    ...
    f'<main class="p-8 max-w-7xl mx-auto w-full">{flash_messages}',
)
...
html = html.replace(
    ...
    '<h3 class="text-xl font-bold">{{ overall_attendance_pct|default("0") }}%</h3>',
)
html = html.replace(
    'style="width: 85%"', 'style="width: {{ overall_attendance_pct|default("0") }}%"'
)

html = html.replace(
    '<h3 class="text-xl font-bold">76%</h3>',
    '<h3 class="text-xl font-bold">{{ avg_marks_pct|default("0") }}%</h3>',
)
html = html.replace(
    'style="width: 76%"', 'style="width: {{ avg_marks_pct|default("0") }}%"'
)

html = html.replace(
    '<h3 class="text-xl font-bold">4</h3>',
    '<h3 class="text-xl font-bold">{{ attendance_records|length|default("0") }}</h3>',
)
...
html = re.sub(
    ...
    '<div class="info-card" onclick="window.location.href=\'{{ url_for(\'student.marks_overview\') }}\'">\n<div class="flex items-center justify-between mb-2">\n<p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Avg Marks</p>',
    html,
)
html = re.sub(
...
html = re.sub(
    ...
    '<div class="info-card" onclick="window.location.href=\'{{ url_for(\'student.lab_index\') }}\'">\n<div class="flex items-center justify-between mb-2">\n<p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Lab Status</p>',
    html,
)
...
# Quick Actions Links
html = html.replace(
    '<button class="bg-white p-4', '<a href="#" class="block bg-white p-4'
)
html = html.replace("</button>", "</a>")

html = html.replace(
    '<p class="font-bold text-[11px] text-slate-900">Raise Complaint</p>\n</a>',
    '<p class="font-bold text-[11px] text-slate-900">Raise Complaint</p>\n</a>',
).replace(
    'href="#" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    'href="{{ url_for(\'student.raise_complaint\') }}" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    1,
)

html = html.replace(
    '<p class="font-bold text-[11px] text-slate-900">Give Feedback</p>\n</a>',
    '<p class="font-bold text-[11px] text-slate-900">Give Feedback</p>\n</a>',
).replace(
    'href="#" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    'href="{{ url_for(\'student.academic_feedback\') }}" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    1,
)

html = html.replace(
    '<p class="font-bold text-[11px] text-slate-900">Track Complaint</p>\n</a>',
    '<p class="font-bold text-[11px] text-slate-900">Track Complaint</p>\n</a>',
).replace(
    'href="#" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    'href="{{ url_for(\'student.track_complaint\') }}" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    1,
)

html = html.replace(
    '<p class="font-bold text-[11px] text-slate-900">Campus Updates</p>\n</a>',
    '<p class="font-bold text-[11px] text-slate-900">Campus Updates</p>\n</a>',
).replace(
    'href="#" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    'href="#updates-section" class="block bg-white p-4 border border-slate-200 rounded-lg shadow-sm hover:border-indigo-500 transition-all group text-left"',
    1,
)
...
# Tickets Summary
html = html.replace(
    '<span class="block text-xl font-extrabold text-slate-900">18</span>',
    '<span class="block text-xl font-extrabold text-slate-900">{{ total_complaints }}</span>',
)
html = html.replace(
    '<span class="text-xs font-bold text-slate-900">13</span>',
    '<span class="text-xs font-bold text-slate-900">{{ resolved_complaints }}</span>',
)
html = html.replace(
    '<span class="text-xs font-bold text-slate-900">5</span>',
    '<span class="text-xs font-bold text-slate-900">{{ pending_complaints }}</span>',
)
...
html = html.replace(
    ...
    '<div class="grid grid-cols-1 md:grid-cols-2 gap-4" id="updates-section">',
)
...
if start_idx != -1 and end_idx != -1:
    ...
    # It looks like:
    stitch_table_pattern = re.compile(
        r'<div class="overflow-x-auto">\s*<table class="w-full border-collapse">.*?</table>\s*</div>',
        re.DOTALL,
    )

    colorful_timetable = colorful_timetable.replace(
        "bg-light bg-opacity-50 border-0 shadow-sm", ""
    )

    # --- Shrink the table sizing to fit into the sleek Stitch layout ---
    # Shrink padding and text in the CSS
    colorful_timetable = colorful_timetable.replace(
        "border-spacing: 12px;", "border-spacing: 6px;"
    )
    colorful_timetable = colorful_timetable.replace(
        "min-width: 1000px;", "min-width: 700px;"
    )  # Make it scrollable sooner, takes up less space
    colorful_timetable = colorful_timetable.replace("padding: 14px;", "padding: 8px;")
    colorful_timetable = colorful_timetable.replace(
        "min-height: 90px;", "min-height: 60px;"
    )
    colorful_timetable = colorful_timetable.replace(
    ...
    colorful_timetable = colorful_timetable.replace(
        "padding-right: 1.5rem;", "padding-right: 0.5rem;"
    )  # Day col padding
...
html = html.replace(
    ...
    "hover:border-indigo-500 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group",
)
html = html.replace(
    "group-hover:bg-indigo-600",
    "group-hover:bg-indigo-600 group-hover:scale-110 group-hover:-rotate-3",
)

# 3. Add Animations to Updates
html = html.replace(
    "hover:bg-slate-50 cursor-pointer",
    "hover:bg-slate-50 hover:shadow-md hover:-translate-y-1 transition-all duration-300 cursor-pointer",
)

# 4. Add Animations to Timetable Classes
html = html.replace(
    'rounded-sm"',
    'rounded-sm hover:scale-105 hover:shadow-md transition-all duration-200 cursor-pointer"',
)

# 5. Make user profile interactive
html = html.replace(
    'class="bg-slate-50 rounded-lg p-3 flex items-center gap-3"',
    'class="bg-slate-50 hover:bg-indigo-50 rounded-lg p-3 flex items-center gap-3 cursor-pointer transition-colors duration-200"',
)
